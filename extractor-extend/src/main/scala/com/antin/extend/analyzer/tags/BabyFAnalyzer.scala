...
import com.antin.extend.util.{DateHelper, JsonHelper}
...
/**
  ...
  * 宝妈和宝爸
  */
class BabyFAnalyzer(ss: SparkSession, ymlpath: String) extends Analyzer[Product, BabyMFModel](ss, ymlpath) {
  ...
  def processDataset: Dataset[BabyMFModel] = {
    ...
    // val resultMDF = ss.sql("select t1.birthTime,t2.xmanId,t1.motherIdNo,t1.xmanId from etl_xman_birthcertificate t1,sehr_xman t2 where t1.motherIdNo is not null and t1.motherIdNo <>'' t1.motherIdNo=t2.idNo")//TODO
    ...
    val resultFDF = ss.sql("select t1.birthTime,t2.xmanId,t1.fatherIdNo from etl_xman_birthcertificate t1,sehr_xman t2 where t1.fatherIdNo is not null and t1.fatherIdNo <>'' and t1.fatherIdNo=t2.idNo")

    //motherAndFather(resultMDF, "宝妈", "9")
    motherAndFather(resultFDF, "宝爸", "10") //TODO
  }
    ...
    val resultModel = dataset.rdd.groupBy(_ (1)).map(m => {
      val maps = m._2.map(r => {
        ...
        try {
          ...
          val startTime = r(0).toString.split(" ")(0)
          val endTime = sdf.format(DateHelper.devYear(sdf.parse(r(0).toString), 6))

          map.put("startTime", startTime)
          map.put("endTime", endTime) //6周岁以下的为宝爸
          map
        } catch {
          case e: Exception => {
            e.printStackTrace()
