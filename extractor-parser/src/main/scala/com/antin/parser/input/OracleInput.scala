...
/**
  ...
  */
class OracleInput(val config: OracleInputConfig, val ss: SparkSession, val oracleResult: RDD[OracleResult]) extends Input with Serializable {
  ...
  def dataframe: (String, DataFrame) = {
    ...
    (config.output, ss.createDataFrame(result, schema))
  }
...
class OracleResult extends InputResult {
